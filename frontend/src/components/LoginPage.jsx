...
export default function LoginPage() {
    ...
    async function loadFeedbacks() {
      try {
        ...
        if (feedbacks && feedbacks.length > 0) {
          setTestimonials(feedbacks.map(f => ({ text: f.text, author: f.author })));
        }
      } catch (err) {
        console.warn('Failed to load feedback from backend:', err);
      }
    }
    loadFeedbacks();
  }, []);
  ...
  const handleStudentLogin = async (e) => {
    ...
    if (isRegisterMode) {
      try {
        await registerStudent(email, studentPassword, studentDisplayName);
        setActiveForm(null);
      } catch (err) {
        showToast(err.message, 'warning');
      }
    ...
    try {
      await loginWithCredentials(email, studentPassword, 'student');
      setActiveForm(null);
    } catch (err) {
      showToast(err.message, 'warning');
    }
  ...
  const handleCounselorLogin = async (e) => {
    e.preventDefault();
    try {
      await loginWithCredentials(counselorId.trim(), counselorKey, 'counselor');
      setActiveForm(null);
    } catch (err) {
      showToast(err.message, 'warning');
    }
  ...
  const handleFeedbackSubmit = async (e) => {
    ...
    try {
      await submitFeedback(feedbackRating, newTestimonial.text, newTestimonial.author);
  ...
  const fillDemoCounselor = () => {
    const demos = [
      { id: 's.jenkins@knust.edu.gh', pass: 'counselorpass' },
      { id: 'm.peterson@knust.edu.gh', pass: 'counselorpass' },
      { id: 'a.rivera@knust.edu.gh', pass: 'counselorpass' }
    ];
        ...
        <div style={{ maxWidth: '850px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px', margin: '0 auto 32px auto', maxWidth: '780px' }}>
            ...
            <div className="premium-card card-blue" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', textAlign: 'left' }}>
              <div>
                <h3 style={{ fontSize: '1.2rem', fontWeight: 700, margin: '0 0 8px 0', color: 'var(--primary-blue)' }}>Student Portal Login</h3>
                <p style={{ fontSize: '0.88rem', color: 'var(--text-muted)', margin: '0 0 24px 0', lineHeight: 1.55 }}>
                  Sign in with your student account. Post threads, book counseling sessions, and choose to consult anonymously with our built-in identity shield.
                </p>
              </div>
              <button
                onClick={() => {
            ...
            <div id="auth-form-card" className={`premium-card ${activeForm === 'student' ? 'card-blue' : 'card-yellow'}`} style={{ maxWidth: '600px', margin: '32px auto 0 auto', textAlign: 'left', animation: 'fadeInUp 0.3s ease' }}>
              <div className="modal-header" style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '12px', marginBottom: '16px' }}>
                <h3 className="modal-title">
                  {activeForm === 'student' && 'Student Login'}
              ...
              {activeForm === 'student' && (
                <form onSubmit={handleStudentLogin} className="modal-body" style={{ padding: 0 }}>
                  ...
                  <button type="submit" className="btn-primary" style={{ marginTop: '10px', width: '100%' }}>
                    <span>{isRegisterMode ? 'Create Account & Enter' : 'Log In'}</span>
                    <IconlyArrowRight size={16} />
                  </button>
                  ...
                  <button
                    ...
                    style={{ marginTop: '10px', width: '100%', background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '0.8rem', cursor: 'pointer', textAlign: 'center' }}
                  >
              ...
              {activeForm === 'counselor' && (
                <form onSubmit={handleCounselorLogin} className="modal-body" style={{ padding: 0 }}>
                  ...
                  <div className="form-group">
                    <label className="form-label">Counselor Email</label>
                    <input
                      type="email"
                      className="form-input"
                      placeholder="e.g. s.jenkins@knust.edu.gh"
                      value={counselorId}
